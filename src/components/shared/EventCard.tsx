...
import { getSavedIds } from "@/services/user/evenet.services";
...
export default function EventCard({
  ...
  useEffect(() => {
    ...
    const fetchSavedIds = async () => {
      const savedIds = await getSavedIds(); // ✅ await inside useEffect
      setIsSaved(savedIds.includes(id)); // update state safely
    };
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex justify-between items-center px-5 mb-4">
        ...
        <SaveEventButton eventId={id} defaultSaved={isSaved} />
      </div>
    </div>
  );
}
